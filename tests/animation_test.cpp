...
{
	return Settings{.animationSpeed = 1.0F};
}
...
auto& getTestDependencyInjectionContext()
{
	static auto ctx = []()
	{
		auto settings = boost::di::bind<Settings>().to(std::make_shared<Settings>(createTestSettings()));
		return boost::di::make_injector(settings,  //
										di::animationService(),
										di::animationManager(),
										mock::di::uiManager(),
										di::sceneService(),
...
TEST_F(AnimationTest, single_property_channel_interpolation)
{
	...
	// act
	animManager->tick(Milliseconds{500});
	animManager->animate(*registry, dummyUI);
...
TEST_F(AnimationTest, play_test)
{
	// arrange
	using namespace animation::dsl;

	KeyframeCollection keyframes;
	keyframes += keyframe(0s) | accessor::point(pointId) / point::radius << 1.0F;
	keyframes += keyframe(1s) | accessor::point(pointId) / point::radius << 2.0F;
...
TEST_F(AnimationTest, interpolator_test)
{
	...
	dummyUI.cameras.resize(1);
	dummyUI.cameras[cameraId]						   = beginCamera;
	...
	// assert
	const Camera& camera = dummyUI.cameras[cameraId];
