...
vec4 applyFXAA(vec2 fragCoord, sampler2D tex)
{
    ...
    vec4 texelNW = texture(tex, (fragCoord + vec2(-1.0, -1.0)) * inverseVP);
    vec4 texelNE = texture(tex, (fragCoord + vec2(1.0, -1.0)) * inverseVP);
    vec4 texelSW = texture(tex, (fragCoord + vec2(-1.0, 1.0)) * inverseVP);
    vec4 texelSE = texture(tex, (fragCoord + vec2(1.0, 1.0)) * inverseVP);
    vec4 texelM  = texture(tex, fragCoord * inverseVP);
    ...
    float alphaA = 0.5 * (
        texture(tex, fragCoord * inverseVP + dir * (1.0 / 3.0 - 0.5)).a +
        texture(tex, fragCoord * inverseVP + dir * (2.0 / 3.0 - 0.5)).a);
    
    float alphaB = alphaA * 0.5 + 0.25 * (
        texture(tex, fragCoord * inverseVP + dir * -0.5).a +
        texture(tex, fragCoord * inverseVP + dir * 0.5).a);
...
void main()
{
    ...
    finalColor = fxaaColor * colDiffuse;
