...
class PlayerManager : public IPlayerManager
{
  ...
  public:
	PlayerManager(std::shared_ptr<Settings> settings,
				  const std::shared_ptr<IManager>& animationManager,
				  const std::shared_ptr<IService>& animationService)
		: settings_(settings), animationManager_(animationManager), animationService_(animationService), animationHandle_({})
	{
	}
