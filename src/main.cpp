...
int main(int, char**)
{
	...
	SDL_Window* window =
		SDL_CreateWindow("Dear ImGui SDL3+OpenGL3 example", (int)(1280 * main_scale), (int)(800 * main_scale), window_flags);
	if (window == nullptr)
	{
	...
	io.ConfigDpiScaleFonts = true;	  // [Experimental] Automatically overwrite style.FontScaleDpi in Begin() when Monitor DPI changes.
...
#endif
	{
		...
		// on those two flags. [If using SDL_MAIN_USE_CALLBACKS: call ImGui_ImplSDL3_ProcessEvent() from your SDL_AppEvent() function]
		SDL_Event event;
